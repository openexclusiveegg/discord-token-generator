# 60 stars for a version using yolov5 and solves locally 😙
import requests, datetime, base64, random, string, json, time

__useragent__ = 'Mozilla/5.0 (Windows NT 6.2; rv:20.0) Gecko/20121202 Firefox/20.0' #Change this to ur own useragent for better results!


class Solver:
    def __init__(self, UID, APIKEY, site_key:str, website_url:str, proxyless:bool, proxy, endpoint_url = 'https://free.nocaptchaai.com/api/solve'):
        self.uid=UID
        self.apikey=APIKEY
        self.site_key=site_key
        self.siteurl=website_url
        self.apiendpoint=endpoint_url

        self.client = requests.Session()
        self.client.proxies.update({'https': f'{proxy}'}) if proxyless !=True else print('Proxy Disabled')

        # getting hcaptcha version
        self.hcaptcha_version = self.client.get('https://hcaptcha.com/1/api.js?render=explicit&onload=hcaptchaOnLoad', 
        headers={
            "accept": "*/*",
    ...
    def __get_captcha__(self):
        """Gets the required information to solve the captcha, the tasklist etc"""

        reqdata = json.loads(self.client.post(f"https://hcaptcha.com/checksiteconfig?v={self.hcaptcha_version}&host={self.siteurl}&sitekey={self.site_key}&sc=1&swa=1", 
    ...
    def __hsw__(self, hsw, c):
        """Opens a headless browser window and gets the nvalue required for solving captcha"""
        try: 
            from selenium import webdriver;from webdriver_manager.core.utils import ChromeType; from webdriver_manager.chrome import ChromeDriverManager
            from selenium.webdriver.chrome.service import Service as ChromeService
            options = webdriver.ChromeOptions()
            options.add_argument("window-size=1,1")
            options.add_argument('--disable-blink-features=AutomationControlled')
            options.add_argument(f'user-agent={__useragent__}')
            ...
            print(f'Error Ocurred when Trying to Initialize browser - {e}')
        try:
            ...
            with open('hsw.js', 'w') as f:
                f.write(f"hsw={hsw}")
            with open('hsw.js', 'r') as f:
               nvalue=driver.execute_script(f.read() + f"return hsw('{c}');")
            driver.quit()
        except Exception as e:
            print('Error in the __hsw__ function come take a look: '+ e)
        return str(nvalue)

    def SolveCaptcha(self):
        """Rewritten from hykii's source code to make it more readable as well as changed some things to improve it"""
        reqdata = self.__get_captcha__()
        key, tasklist = reqdata["key"], reqdata["tasklist"]
        __images__ = {}
        t_ = {}
        attempts = 0

        for nig in tasklist:
           url, task_key = str(nig["datapoint_uri"]), str(nig["task_key"])
           __images__[attempts], t_[url] = url, task_key
           attempts += 1

        api = requests.post(self.apiendpoint, json={
        "images": __images__,        
        "target": reqdata["requester_question"]["en"],
        "data_type": "url",
        "site": f'{self.siteurl}',
        "site_key": f'{self.site_key}'
         }, headers={
        ...
        if api.status_code==201:
            if "url" in api.text:
                __api_url__, __solved__, attempts, __answers__ = api.json()["url"], None, 0, {}
                time.sleep(2.5)
                while True:
                    __solved__ = requests.get(__api_url__).text
                    if "solved" in __solved__: 
                        __solved__ = json.loads(__solved__)
                        break
                    attempts += 1
                    time.sleep(5)
                for d in __images__:
                    if str(d) in __solved__["solution"]: __answers__[t_[__images__[d]]] = "true"
                    else: 
                        __answers__[t_[__images__[d]]] = "false"
            else:
                print(api.text)
        else:
            print(api.text)
        n = self.__hsw__(self.hsw, reqdata["c"]["req"])
